...
public class ScheduleService {
  ...
  @Transactional
  public ScheduleResponseDto addSchedule(
      Long userId, Long familyId, String title, String memo, LocalDate date, String authorName) {
  ...
  public ScheduleResponseDto updateSchedule(
      Long scheduleId, Long userId, String title, String memo, LocalDate date) {
    if (title == null || title.isEmpty()) {
      throw new IllegalArgumentException("제목은 필수 입력 항목입니다.");
    }
  ...
  public void deleteSchedule(Long scheduleId, Long userId) {
    ...
    schedule.setIsDeleted(true); // 필드 이름 변경에 따라 수정
    scheduleRepository.save(schedule); // 소프트 삭제 저장
  }

  @Transactional(readOnly = true)
  public List<ScheduleResponseDto> findAllSchedules() {
    List<Schedule> schedules = scheduleRepository.findAllByIsDeletedFalse(); // 삭제되지 않은 일정만 반환
