...
public class LetterController {
  ...
  public ResponseEntity<Map<String, String>> generatePresignedUrl(
    ...
    Map<String, String> response =
        letterService.generatePresignedUrl(familyId, extractUserId(bearerToken), receiverId);
    return ResponseEntity.status(HttpStatus.CREATED).body(response);
  }

  @PostMapping("/file")
  @Operation(summary = "편지 전송(아직 받은 사람은 확인 x)", description = "보낸 편지의 정보를 전송 및 db에 저장합니다.")
  public ResponseEntity<LetterResponse> saveLetter(
      @RequestHeader("Authorization") String bearerToken, @RequestBody LetterReqDto request) {
  ...
  public ResponseEntity<LetterResponse> saveTextLetter(
      @RequestHeader("Authorization") String bearerToken,
  ...
  public ResponseEntity<ScheduleLetterResponse> saveTextLetter(
      @RequestHeader("Authorization") String bearerToken,
      @RequestBody ScheduleLetterRequestDto request) {

    ScheduleLetterResponse response =
        letterService.saveScheduleLetter(extractUserId(bearerToken), request);
    return ResponseEntity.status(HttpStatus.CREATED).body(response);
  }

  @GetMapping("/saved")
  @Operation(summary = "모든 편지 조회", description = "사용자가 보관한 모든 편지를 조회합니다.")
  public ResponseEntity<List<LetterResponse>> getSavedLetters(
      @RequestHeader("Authorization") String bearerToken) {
  ...
  public ResponseEntity<List<LetterResponse>> getNullSavedLetters(
      @RequestHeader("Authorization") String bearerToken) {
  ...
  @Operation(
      summary = "편지 조회용 Presigned Url 생성",
      description = "편지 파일을 다운받기 위한 Presigned URL을 생성합니다.")
